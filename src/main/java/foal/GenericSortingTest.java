...
public static void testRandomly(int runs) {
	...
	for (int run=0; run<runs; run++) {
		...
		foal.matrix.DoubleMatrix2D A1 = new foal.matrix.impl.DenseDoubleMatrix2D(size,size);
		foal.matrix.DoubleMatrix2D P1 = A1.viewPart(from,from,size-to,size-to);
		...
		foal.matrix.DoubleMatrix2D A2 = A1.copy();
		foal.matrix.DoubleMatrix2D P2 = A2.viewPart(from,from,size-to,size-to);

		int c = 0;
		foal.matrix.DoubleMatrix2D S1 = foal.matrix.doublealgo.Sorting.quickSort.sort(P1,c);
		foal.matrix.DoubleMatrix2D S2 = foal.matrix.doublealgo.Sorting.mergeSort.sort(P2,c);
