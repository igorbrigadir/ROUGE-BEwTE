...
*/
package foal.matrix.doublealgo;

import foal.function.IntComparator;
import foal.matrix.DoubleFactory2D;
import foal.matrix.DoubleFactory3D;
import foal.matrix.DoubleMatrix1D;
import foal.matrix.DoubleMatrix2D;
import foal.matrix.DoubleMatrix3D;
...
*/
public DoubleMatrix1D sort(final DoubleMatrix1D vector, final foal.function.DoubleComparator c) {
	int[] indexes = new int[vector.size()]; // row indexes to reorder instead of matrix itself
	for (int i=indexes.length; --i >= 0; ) indexes[i] = i;
...
public DoubleMatrix2D sort(DoubleMatrix2D matrix, final double[] aggregates) {
	...
	foal.function.IntComparator comp = new foal.function.IntComparator() {
	...
	foal.Swapper swapper = new foal.Swapper() {
...
public static void zdemo3() {
	...
	foal.function.DoubleComparator comp = new foal.function.DoubleComparator() {
		public int compare(double a, double b) {
			double as = Math.sin(a); double bs = Math.sin(b);
			return as < bs ? -1 : as == bs ? 0 : 1;
		}
...
public static void zdemo7(int rows, int columns, boolean print) {
	...
	DoubleMatrix2D A = foal.matrix.DoubleFactory2D.dense.make(rows,columns);
	...
	foal.Timer timer = new foal.Timer().start();
	quickSort.sort(A,0);
	timer.stop().display();
