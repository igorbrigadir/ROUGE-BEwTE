...
*/
public Object aggregate(ObjectMatrix1D<T> other, foal.function.ObjectObjectFunction<T> aggr, foal.function.ObjectObjectFunction<T> f) {
	checkSize(other);
	if (size==0) return null;
...
*/
public ObjectMatrix1D<T> assign(foal.function.ObjectFunction<T> function) {
	for (int i=size; --i >= 0; ) {
...
@see cern.jet.math.Functions
*/
public ObjectMatrix1D<T> assign(ObjectMatrix1D<T> y, foal.function.ObjectObjectFunction<T> function) {
	checkSize(y);
	for (int i=size; --i >= 0; ) {
...
*/
public void getNonZeros(IntArrayList indexList, ObjectArrayList<T> valueList) {
...
@return the new view.
*/
public ObjectMatrix1D<T> viewSelection(foal.function.ObjectProcedure<T> condition) {
...
For further information, see {@link foal.matrix.objectalgo.Sorting#sort(ObjectMatrix1D)}.
For more advanced sorting functionality, see {@link foal.matrix.objectalgo.Sorting}.
@return a new sorted vector (matrix) view.
*/
public ObjectMatrix1D<T> viewSorted() {
	return foal.matrix.objectalgo.Sorting.mergeSort.sort(this);
}
/**
