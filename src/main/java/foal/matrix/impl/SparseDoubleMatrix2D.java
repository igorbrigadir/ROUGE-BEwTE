...
*/
package foal.matrix.impl;

import foal.map.AbstractIntDoubleMap;
...
Uses a {@link foal.map.IntDoubleHashMap}, which is a compact and performant hashing technique.
<p>
 ...
 * All entries are initially <tt>0</tt>.
 * For details related to memory usage see {@link foal.map.IntDoubleHashMap}.
 * 
 * @param rows the number of rows the matrix shall have.
 * @param columns the number of columns the matrix shall have.
...
public SparseDoubleMatrix2D(int rows, int columns, int initialCapacity, double minLoadFactor, double maxLoadFactor) {
	setUp(rows,columns); 
	this.elements = new IntDoubleHashMap(initialCapacity, minLoadFactor, maxLoadFactor);
}
/**
...
public DoubleMatrix2D assign(final DoubleMatrix2D y, foal.function.DoubleDoubleFunction function) {
	...
	if (function instanceof cern.jet.math.PlusMult) { // x[i] = x[i] + alpha*y[i]
		...
		y.forEachNonZero(
			new foal.function.IntIntDoubleFunction() {
	...
	if (function==cern.jet.math.Functions.mult) { // x[i] = x[i] * y[i]
		this.elements.forEachPair(
			new foal.function.IntDoubleProcedure() {
