...
import foal.map.AbstractIntObjectMap;
...
Uses a {@link foal.map.IntObjectHashMap}, which is a compact and performant hashing technique.
<p>
 ...
 * All entries are initially <tt>null</tt>.
 * For details related to memory usage see {@link foal.map.IntObjectHashMap}.
 * 
 * @param rows the number of rows the matrix shall have.
 * @param columns the number of columns the matrix shall have.
...
public SparseObjectMatrix2D(int rows, int columns, int initialCapacity, double minLoadFactor, double maxLoadFactor) {
	setUp(rows,columns); 
	this.elements = new IntObjectHashMap(initialCapacity, minLoadFactor, maxLoadFactor);
}
/**
