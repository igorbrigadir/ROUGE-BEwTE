...
import foal.matrix.impl.AbstractMatrix2D;
import foal.matrix.impl.DenseDoubleMatrix1D;
import foal.matrix.impl.DenseDoubleMatrix2D;
/**
...
*/
public double aggregate(foal.function.DoubleDoubleFunction aggr, foal.function.DoubleFunction f) {
...
@see cern.jet.math.Functions
*/
public double aggregate(DoubleMatrix2D other, foal.function.DoubleDoubleFunction aggr, foal.function.DoubleDoubleFunction f) {
	checkShape(other);
	if (size()==0) return Double.NaN;
...
@see cern.jet.math.Functions
*/
public DoubleMatrix2D assign(foal.function.DoubleFunction function) {
	for (int row=rows; --row >= 0; ) {
		for (int column=columns; --column >= 0; ) {
...
@see cern.jet.math.Functions
*/
public DoubleMatrix2D assign(DoubleMatrix2D y, foal.function.DoubleDoubleFunction function) {
	checkShape(y);
	for (int row=rows; --row >= 0; ) {
		for (int column=columns; --column >= 0; ) {
...
public boolean equals(double value) {
	return foal.matrix.linalg.Property.DEFAULT.equals(this,value);
}
/**
...
public boolean equals(Object obj) {
	...
	return foal.matrix.linalg.Property.DEFAULT.equals(this,(DoubleMatrix2D) obj);
}
/**
 ...
 * @return <tt>this</tt> (for convenience only).
 */
public DoubleMatrix2D forEachNonZero(final foal.function.IntIntDoubleFunction function) {
	for (int row=rows; --row >= 0;) {
		for (int column=columns; --column >= 0;) {
 ...
 * @see foal.matrix.doublealgo.Formatter
 */
public String toString() {
	return new foal.matrix.doublealgo.Formatter().toString(this);
}
/**
...
For further information, see {@link foal.matrix.doublealgo.Sorting#sort(DoubleMatrix2D,int)}.
For more advanced sorting functionality, see {@link foal.matrix.doublealgo.Sorting}.
@return a new sorted vector (matrix) view.
@throws IndexOutOfBoundsException if <tt>column < 0 || column >= columns()</tt>.
*/
public DoubleMatrix2D viewSorted(int column) {
	return foal.matrix.doublealgo.Sorting.mergeSort.sort(this,column);
}
/**
 ...
 */
private boolean xforEach(final foal.function.DoubleProcedure procedure) {
	for (int row=rows; --row >= 0;) {
		for (int column=columns; --column >= 0;) {
			if (!procedure.apply(getQuick(row,column))) return false;
		}
...
*/
public void zAssign8Neighbors(DoubleMatrix2D B, foal.function.Double9Function function) {
