...
public void decompose(DoubleMatrix2D A) {
	...
	foal.list.IntArrayList nonZeroIndexes = new foal.list.IntArrayList(); // sparsity
...
public void solve(DoubleMatrix2D B) {
	...
	foal.list.IntArrayList nonZeroIndexes = new foal.list.IntArrayList(); // sparsity
	DoubleMatrix1D Browk = foal.matrix.DoubleFactory1D.dense.make(nx); // blocked row k
	...
	for (int k = n-1; k >= 0; k--) {
		...
		// blocking
		if (Browk==null) Browk = foal.matrix.DoubleFactory1D.dense.make(B.columns());
		Browk.assign(Brows[k]);
...
public String toString() {
	...
	buf.append("\npivot = ");
	try { buf.append(String.valueOf(new foal.list.IntArrayList(this.getPivot())));} 
	catch (IllegalArgumentException exc) { buf.append(unknown+exc.getMessage()); }
	...
	buf.append("\n\ninverse(A) = ");
	DoubleMatrix2D identity = foal.matrix.DoubleFactory2D.dense.identity(LU.rows());
