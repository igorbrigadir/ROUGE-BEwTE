...
*/
public class Property extends foal.PersistentObject {
	/**
...
public void checkRectangular(DoubleMatrix2D A) {
	if (A.rows() < A.columns()) {
		throw new IllegalArgumentException("Matrix must be rectangular: "+foal.matrix.doublealgo.Formatter.shape(A));
	}
}
/**
...
public void checkSquare(DoubleMatrix2D A) {
	if (A.rows() != A.columns()) throw new IllegalArgumentException("Matrix must be square: "+foal.matrix.doublealgo.Formatter.shape(A));
}
/**
...
/**
 */
protected static String get(foal.list.ObjectArrayList list, int index) {
...
public boolean isOrthogonal(DoubleMatrix2D A) {
	checkSquare(A);
	return equals(A.zMult(A,null,1,0,false,true), foal.matrix.DoubleFactory2D.dense.identity(A.rows()));
}
/**
...
public String toString(DoubleMatrix2D A) {
	final foal.list.ObjectArrayList names = new foal.list.ObjectArrayList();
	final foal.list.ObjectArrayList values = new foal.list.ObjectArrayList();
	...
	foal.GenericSorting.quickSort(0,names.size(),comp,swapper);
