 ...
 * Returns sqrt(a^2 + b^2) without under/overflow.
 */
protected static foal.function.DoubleDoubleFunction hypotFunction() {
	return new foal.function.DoubleDoubleFunction() {
...
public DoubleMatrix2D permuteRows(final DoubleMatrix2D A, int[] indexes, int[] work) {
	...
	*/
	...
	foal.GenericPermuting.permute(indexes, swapper, work, null);
	return A;
}
/**
...
public DoubleMatrix2D pow(DoubleMatrix2D A, int p) {
	...
	int k = foal.bitvector.QuickBitVector.mostSignificantBit(p); // index of highest bit in state "true"
...
public String toString(DoubleMatrix2D matrix) {
	final foal.list.ObjectArrayList<String> names = new foal.list.ObjectArrayList<String>();
	final foal.list.ObjectArrayList<String> values = new foal.list.ObjectArrayList<String>();
