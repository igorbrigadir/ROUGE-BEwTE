...
public QRDecomposition (DoubleMatrix2D A) {
	...
	Rdiag = A.like1D(n);
	//Rdiag = new double[n];
	foal.function.DoubleDoubleFunction hypot = Algebra.hypotFunction();
...
public String toString() {
	...
	buf.append("\n\npseudo inverse(A) = ");
	try { buf.append(String.valueOf(this.solve(foal.matrix.DoubleFactory2D.dense.identity(QR.rows()))));} 
	catch (IllegalArgumentException exc) { buf.append(unknown+exc.getMessage()); }
