...
@see cern.jet.math.Functions
*/
public double aggregate(DoubleMatrix3D other, foal.function.DoubleDoubleFunction aggr, foal.function.DoubleDoubleFunction f) {
	checkShape(other);
	if (size()==0) return Double.NaN;
...
@see cern.jet.math.Functions
*/
public DoubleMatrix3D assign(foal.function.DoubleFunction function) {
	for (int slice=slices; --slice >= 0; ) {
		for (int row=rows; --row >= 0; ) {
			for (int column=columns; --column >= 0; ) {
...
@see cern.jet.math.Functions
*/
public DoubleMatrix3D assign(DoubleMatrix3D y, foal.function.DoubleDoubleFunction function) {
	checkShape(y);
	for (int slice=slices; --slice >= 0; ) {
		for (int row=rows; --row >= 0; ) {
			for (int column=columns; --column >= 0; ) {
...
public boolean equals(Object obj) {
	...
	return foal.matrix.linalg.Property.DEFAULT.equals(this,(DoubleMatrix3D) obj);
}
/**
...
For further information, see {@link foal.matrix.doublealgo.Sorting#sort(DoubleMatrix3D,int,int)}.
...
public DoubleMatrix3D viewSorted(int row, int column) {
	return foal.matrix.doublealgo.Sorting.mergeSort.sort(this,row,column);
}
/**
...
*/
public void zAssign27Neighbors(DoubleMatrix3D B, foal.function.Double27Function function) {
