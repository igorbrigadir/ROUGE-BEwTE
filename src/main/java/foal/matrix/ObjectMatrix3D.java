...
import foal.list.ObjectArrayList;
import foal.matrix.impl.AbstractMatrix3D;
/**
...
*/
public T aggregate(foal.function.ObjectObjectFunction<T> aggr, foal.function.ObjectFunction<T> f) {
...
@see cern.jet.math.Functions
*/
public T aggregate(ObjectMatrix3D<T> other, foal.function.ObjectObjectFunction<T> aggr, foal.function.ObjectObjectFunction<T> f) {
	checkShape(other);
	if (size()==0) return null;
...
@see cern.jet.math.Functions
*/
public ObjectMatrix3D<T> assign(foal.function.ObjectFunction<T> function) {
	for (int slice=slices; --slice >= 0; ) {
		for (int row=rows; --row >= 0; ) {
			for (int column=columns; --column >= 0; ) {
...
@see cern.jet.math.Functions
*/
public ObjectMatrix3D<T> assign(ObjectMatrix3D<T> y, foal.function.ObjectObjectFunction<T> function) {
	checkShape(y);
	for (int slice=slices; --slice >= 0; ) {
		for (int row=rows; --row >= 0; ) {
			for (int column=columns; --column >= 0; ) {
 ...
 */
public T get(int slice, int row, int column) {
...
*/
public void getNonZeros(IntArrayList sliceList, IntArrayList rowList, IntArrayList columnList, ObjectArrayList<T> valueList) {
	sliceList.clear(); 
	rowList.clear(); 
	columnList.clear(); 
	valueList.clear();
 ...
 */
protected abstract ObjectMatrix2D<T> like2D(int rows, int columns, int rowZero, int columnZero, int rowStride, int columnStride);
/**
 ...
 */
public T[][][] toArray() {
	T[][][] values = (T[][][])new Object[slices][rows][columns];
	for (int slice=slices; --slice >= 0;) {
		T[][] currentSlice = values[slice];
		for (int row=rows; --row >= 0;) {
			T[] currentRow = currentSlice[row];
 ...
 * @see foal.matrix.objectalgo.Formatter
 */
@Override
public String toString() {
	return new foal.matrix.objectalgo.Formatter().toString(this);
}
/**
...
@return the new view.
*/
public ObjectMatrix3D<T> viewSelection(ObjectMatrix2DProcedure condition) {
...
For further information, see {@link foal.matrix.objectalgo.Sorting#sort(ObjectMatrix3D,int,int)}.
For more advanced sorting functionality, see {@link foal.matrix.objectalgo.Sorting}.
...
public ObjectMatrix3D<T> viewSorted(int row, int column) {
	return foal.matrix.objectalgo.Sorting.mergeSort.sort(this,row,column);
}
/**
 ...
 */
private boolean xforEach(final foal.function.ObjectProcedure<T> procedure) {
	for (int slice=slices; --slice >= 0;) {
		for (int row=rows; --row >= 0;) {
			for (int column=columns; --column >= 0;) {
 ...
 * @return <tt>false</tt> if the procedure stopped before all elements where iterated over, <tt>true</tt> otherwise. 
 */
private boolean xforEachCoordinate(final foal.function.IntIntIntProcedure procedure) {
	for (int column=columns; --column >= 0;) {
		for (int slice=slices; --slice >=0;) {
			for (int row=rows; --row >= 0;) {
				if (!procedure.apply(slice,row,column)) return false;
