...
*/
package foal.matrix.objectalgo;

import foal.function.IntComparator;
import foal.matrix.ObjectMatrix1D;
import foal.matrix.ObjectMatrix2D;
import foal.matrix.ObjectMatrix3D;
/**
...
*/
public class Sorting extends foal.PersistentObject {
	/**
	...
	public static final Sorting mergeSort = new Sorting() { // override quicksort with mergesort
		protected void runSort(int[] a, int fromIndex, int toIndex, IntComparator c) {
			foal.Sorting.mergeSort(a,fromIndex,toIndex,c);
		}
		protected void runSort(int fromIndex, int toIndex, IntComparator c, foal.Swapper swapper) {
			foal.GenericSorting.mergeSort(fromIndex, toIndex, c, swapper);
		}
	};
/**
...
protected Sorting() {}
protected void runSort(int[] a, int fromIndex, int toIndex, IntComparator c) {
	foal.Sorting.quickSort(a,fromIndex,toIndex,c);
}
protected void runSort(int fromIndex, int toIndex, IntComparator c, foal.Swapper swapper) {
	foal.GenericSorting.quickSort(fromIndex, toIndex, c, swapper);
}
/**
...
*/
public <T> ObjectMatrix1D<T> sort(final ObjectMatrix1D<T> vector) {
...
*/
public <T> ObjectMatrix1D<T> sort(final ObjectMatrix1D<T> vector, final java.util.Comparator<T> c) {
	int[] indexes = new int[vector.size()]; // row indexes to reorder instead of matrix itself
	for (int i=indexes.length; --i >= 0; ) indexes[i] = i;
...
*/
public <T> ObjectMatrix2D<T> sort(ObjectMatrix2D<T> matrix, int column) {
...
@return a new matrix view having rows sorted as specified.
		<b>Note that the original matrix is left unaffected.</b>
*/
public <T> ObjectMatrix2D<T> sort(final ObjectMatrix2D<T> matrix, final ObjectMatrix1DComparator<T> c) {
	int[] rowIndexes = new int[matrix.rows()]; // row indexes to reorder instead of matrix itself
	for (int i=rowIndexes.length; --i >= 0; ) rowIndexes[i] = i;
...
*/
public <T> ObjectMatrix3D<T> sort(ObjectMatrix3D<T> matrix, int row, int column) {
	...
	final ObjectMatrix1D<T> sliceView = matrix.viewRow(row).viewColumn(column);
	IntComparator comp = new IntComparator() {  
		public int compare(int a, int b) {
...
@return a new matrix view having slices sorted as specified.
		<b>Note that the original matrix is left unaffected.</b>
*/
public <T> ObjectMatrix3D<T> sort(final ObjectMatrix3D<T> matrix, final ObjectMatrix2DComparator<T> c) {
	int[] sliceIndexes = new int[matrix.slices()]; // indexes to reorder instead of matrix itself
	for (int i=sliceIndexes.length; --i >= 0; ) sliceIndexes[i] = i;
