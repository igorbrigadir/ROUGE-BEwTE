...
@see cern.jet.math.Functions
*/
public T aggregate(ObjectMatrix2D<T> other, foal.function.ObjectObjectFunction<T> aggr, foal.function.ObjectObjectFunction<T> f) {
	checkShape(other);
	if (size()==0) return null;
...
@see cern.jet.math.Functions
*/
public ObjectMatrix2D<T> assign(foal.function.ObjectFunction<T> function) {
	for (int row=rows; --row >= 0; ) {
		for (int column=columns; --column >= 0; ) {
...
@see cern.jet.math.Functions
*/
public ObjectMatrix2D<T> assign(ObjectMatrix2D<T> y, foal.function.ObjectObjectFunction<T> function) {
	checkShape(y);
	for (int row=rows; --row >= 0; ) {
		for (int column=columns; --column >= 0; ) {
...
*/
public void getNonZeros(IntArrayList rowList, IntArrayList columnList, ObjectArrayList<T> valueList) {
	rowList.clear(); 
	columnList.clear(); 
	valueList.clear();
	int r = rows;
 ...
 */
public T[][] toArray() {
	T[][] values = (T[][])new Object[rows][columns];
	for (int row=rows; --row >= 0;) {
		T[] currentRow = values[row];
		for (int column=columns; --column >= 0;) {
...
@return the new view.
*/
public ObjectMatrix2D<T> viewSelection(ObjectMatrix1DProcedure condition) {
...
For further information, see {@link foal.matrix.objectalgo.Sorting#sort(ObjectMatrix2D,int)}.
...
public ObjectMatrix2D<T> viewSorted(int column) {
	return foal.matrix.objectalgo.Sorting.mergeSort.sort(this,column); 
}
/**
