...
import foal.function.ObjectProcedure;
/**
 ...
 * @see java.util.Arrays
 */
public int binarySearchFromTo(T key, int from, int to) {
	int low = from;
	int high = to;
 ...
 */
public int binarySearchFromTo(T key, int from, int to, java.util.Comparator comparator) {
	return foal.Sorting.binarySearchFromTo(this.elements,key,from,to,comparator);
}
/**
 ...
 */
public Object clone() {
	ObjectArrayList<T> v = (ObjectArrayList<T>)super.clone();
	v.elements = (T[]) elements.clone();
	return v;
}
/**
 ...
 */
public boolean contains(T elem, boolean testForEquality) {
 ...
 */
public void delete(T element, boolean testForEquality) {
 ...
 */
public ObjectArrayList<T> elements(T[] elements) {
...
public void ensureCapacity(int minCapacity) {
	elements = (T[])foal.Arrays.ensureCapacity((Object[])elements,minCapacity);
}
/**
 ...
 */
public int indexOf(T element, boolean testForEquality) {
 ...
 */
public int indexOfFromTo(T element, int from, int to, boolean testForEquality) {
	if (size==0) return -1;
 ...
 */
public int lastIndexOf(T element, boolean testForEquality) {
 ...
 * @exception IndexOutOfBoundsException index is out of range (<tt>size()&gt;0 && (from&lt;0 || from&gt;to || to&gt;=size())</tt>).
 */
public int lastIndexOfFromTo(T element, int from, int to, boolean testForEquality) {
	if (size==0) return -1;
	checkRangeFromTo(from, to, size);
 ...
 * @see Comparator
 */
public void mergeSortFromTo(int from, int to, java.util.Comparator<? super T> c) {
	if (size==0) return;
	checkRangeFromTo(from, to, size);
...
public ObjectArrayList<T> partFromTo(int from, int to) {
	if (size==0) return new ObjectArrayList<T>(0);

	checkRangeFromTo(from, to, size);

	T[] part = (T[])new Object[to-from+1];
...
public void quickSortFromTo(int from, int to) {
	if (size==0) return;
	checkRangeFromTo(from, to, size);
	foal.Sorting.quickSort(elements, from, to+1);
}
/**
 * Sorts the receiver according
 * to the order induced by the specified comparator.  All elements in the
...
public void quickSortFromTo(int from, int to, java.util.Comparator c) {
	if (size==0) return;
	checkRangeFromTo(from, to, size);
	foal.Sorting.quickSort(elements, from, to+1, c);
}
/**
...
public ObjectArrayList<T> times(int times) {
	ObjectArrayList<T> newList = new ObjectArrayList<T>(times*size);
	for (int i=times; --i >= 0; ) {
...
public java.util.ArrayList<T> toList() {
	int mySize = size();
	T[] theElements = elements;
	java.util.ArrayList<T> list = new java.util.ArrayList<T>(mySize);
...
public String toString() {
	return foal.Arrays.toString(partFromTo(0, size()-1).elements());
}
/**
...
public void trimToSize() {
	elements = (T[])foal.Arrays.trimToCapacity((T[])elements,size());
}
}
