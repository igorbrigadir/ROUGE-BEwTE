...
import foal.function.ByteComparator;
import foal.function.CharComparator;
import foal.function.DoubleComparator;
import foal.function.FloatComparator;
import foal.function.IntComparator;
import foal.function.LongComparator;
import foal.function.ShortComparator;
/**
 ...
 * @see foal.matrix.doublealgo.Sorting
...
if (numNegZeros != 0) {
	int j = new foal.list.DoubleArrayList(a).binarySearchFromTo(0.0d, fromIndex, n-1); // posn of ANY zero
	do {
		j--;
	} while (j>=0 && a[j]==0.0d);
...
// Postprocessing phase: change 0.0's to -0.0's as required
if (numNegZeros != 0) {
	int j = new foal.list.FloatArrayList(a).binarySearchFromTo(0.0f, fromIndex, n-1); // posn of ANY zero
	do {
		j--;
	} while (j>=0 && a[j]==0.0f);
