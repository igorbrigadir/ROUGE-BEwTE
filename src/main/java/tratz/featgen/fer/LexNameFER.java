...
/**
 * Feature extraction rule for extracting 'lexicographer filenames' (see lexnames file)
 * from WordNet
 *
 */
public class LexNameFER extends AbstractWordNetFER {
		...
		if(pos != null) {
			...
			if(entry != null) {
				...
				for(int i = 0; i < senses.length && i < mMaxSenseNum;i++) {
					for(Key key : senses[i].getKeys()) {
						productions.add(key.getLexFileName());
					}
				}
			}
		}
